...
function doSavePayAndShipmentInfo(saveType) {
  ...
  //保存自提点
    ...
    $("#pickname").val(pick_name);
    $("#regionId_default").val(regionId);
    alert(regionId);
...
function doSaveDialogPickSite() { //保存自提点
...
function doEditPickSiteDate(venderId) {
 ...
 var node =  document.getElementById("regionId_default");
 doEditPickReigon(node);
}
...
function doEditPickReigon(obj) {
  var actionUrl = "http://www.taotao.com/service/pickview/list"; //根据关联区id获取自提点列表
  var payId = $("#payment-list .payment-item.item-selected").attr("payid");
  ...
  jQuery.ajax({
    ...
    success: function(dataResult, textStatus) {
      ...
      var pickViewList = dataResult.data;
      if (pickViewList.length == 0) {
        alert("自提点获取异常，请重新选择或稍后尝试");
        goOrder();
      }
      var pickSiteListHTML = "";
      if (pickViewList.length != 0) {
        for (var i = 0; i < pickViewList.length; i++) {
          pickSiteListHTML = pickSiteListHTML + "<div class='site-item";
          if (pickViewList[i].selected == true) {
            pickSiteListHTML = pickSiteListHTML + " site-item-selected";
          }
          //add by zhuqingjie 添加自提点不可用标记
          if (pickViewList[i].cabinetAvailable == false) {
            pickSiteListHTML = pickSiteListHTML + " site-item-disabled";
          }
          //end add
          pickSiteListHTML = pickSiteListHTML + "' pickName='" + pickViewList[i].name + "' pickid='" + pickViewList[i].id + "' > <div class='site-in-short' onclick='doSelectPicksite(this)' >";
          pickSiteListHTML = pickSiteListHTML + pickViewList[i].name;
          if (pickViewList[i].used == true) {
            pickSiteListHTML = pickSiteListHTML + "[常用]";
          }
          if (pickViewList[i].cabinetAvailable == false) {
            pickSiteListHTML = pickSiteListHTML + "[已满]";
          }
          if (pickViewList[i].limitKeyword == "1") {
            pickSiteListHTML = pickSiteListHTML + "[限]";
          }
          if (pickViewList[i].limitKeyword == "1" && pickViewList[i].specialRemark != "") {
            pickSiteListHTML = pickSiteListHTML + pickViewList[i].specialRemark;
          }
          ...
          pickSiteListHTML = pickSiteListHTML + pickViewList[i].address;
          pickSiteListHTML = pickSiteListHTML + " </span> ";
          pickSiteListHTML = pickSiteListHTML + " <a class='ftx-05 map-link' target='_blank' href='#'>获取帮助</a> ";
          pickSiteListHTML = pickSiteListHTML + " </div> <div class='clr'></div></div>  ";
        }
      }
      $("#selfpick_siteDiv .pick-sites").html(pickSiteListHTML);
      if (pickViewList.length >= 5) {
    ...
    success: function(dataResult, textStatus) {
      ...
      if (pickViewList.length != 0) {
        for (var i = 0; i < pickViewList.length; i++) {
          ...
          //end add
          pickSiteListHTML = pickSiteListHTML + "' pickid='" + pickViewList[i].pickId + "' > <div class='site-in-short' onclick='doSelectPicksite(this)' >";
          pickSiteListHTML = pickSiteListHTML + pickViewList[i].pickName;
          if (pickViewList[i].used == true) {
            pickSiteListHTML = pickSiteListHTML + "[常用]";
          }
          if (pickViewList[i].cabinetAvailable == false) {
          ...
          pickSiteListHTML = pickSiteListHTML + pickViewList[i].address;
          pickSiteListHTML = pickSiteListHTML + " </span> ";
          pickSiteListHTML = pickSiteListHTML + " <a class='ftx-05 map-link' target='_blank' href='" + pickViewList[i].mapUrl + "'>" + pickViewList[i].helpMessage + "</a> ";
          pickSiteListHTML = pickSiteListHTML + " </div> <div class='clr'></div></div>  ";
