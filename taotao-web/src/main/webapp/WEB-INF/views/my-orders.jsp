		...
		<th>
			<select id="submitDate" name="" class="sele">
				<option value="1" selected="">最近三个月</option>
								  <option value="2">最近二个月</option>
								  <option value="2013">最近一个月</option>
								  <option value="2012">所有的</option>
							</select>
		</th>
		<th>
...
function ensureMoveOrderToRecycle(orderId){
	var con = confirm("您确定要删除么吗？");
	if (con==true){
		window.location.href="http://www.taotao.com/order/deleteOrderByOrderId/"+orderId+".html";
	}
};

    $("#submitDate").change(function () {
        var sDate = $("#submitDate option[@selected]").val();
        window.location = 'http://www.taotao.com/service/order/queryOrdersByUser';
    });
	...
	$("#orderState").val(4096);
  	...
  	<c:forEach items="${easyUIResult.rows}" var="order">
	    ...
	    <tbody id="tb-2538292730">
	        ...
	        <tr class="tr-th">
	            <td colspan="6">
	                <span class="tcol1">
	                    订单编号:
	                    <a name="orderIdLinks" id="idUrl2538292730" target="_blank" href="http://www.taotao.com/order/queryOrder/${order.orderId}.html" clstag="click|keycount|orderinfo|order_num">${ order.orderId}</a>
	        ...
	        <tr id="track2538292730" oty="0,1,70" class="tr-td">
	            ...
	            <td>
	                <span class="ftx-03"> <br>${order.updateTime}</span>
	                <input type="hidden" id="datasubmit-2538292730" value="">
	            </td>
	            ...
	            <a target="_blank" href="http://www.taotao.com/order/queryOrder/${order.orderId}.html" clstag="click|keycount|orderinfo|order_check">查看</a>
	            <span id="order_comment"></span>
	            <span class="pop-recycle-a">|<a href="javascript:void(0)" clstag="click|keycount|orderinfo|order_del" onclick="ensureMoveOrderToRecycle(${order.orderId});">删除</a></span>
	            <span id="doi2538292730"><br><a href="http://www.taotao.com/my-order-comment.jsp" target="_blank" clstag="click|keycount|orderinfo|order_comment">评价晒单</a><br></span>
	            ...
	            <a class="btn-again" clstag="click|keycount|orderlist|buy" href="http://www.taotao.com/cart/add/${order.orderItems[0].itemId}.html" target="_blank">还要买</a>
