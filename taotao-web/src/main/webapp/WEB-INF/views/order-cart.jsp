...
<div class="step-cont" id="skuPayAndShipment-cont">
   ...
   <!--满赠套装商品-->
	 ...
	 <c:set var="totalprice" value="${totalprice + cart.price/1000 *cart.num }"/>   						
        <div class="goods-item goods-item-extra">
    
				<div class="p-img">
					<a target="_blank" href="http://www.taotao.com/item/${cart.id}.html"><img src="${cart.image}" alt=""></a>
				</div>
				<div class="goods-msg">					
					<div class="p-name">
					     <a href="http://www.taotao.com/item/${cart.id}.html" target="_blank">
					           ${cart.title }				     
					     </a>
					</div>
          <div class="p-price">
			  <!--增加预售金额显示 begin   预售分阶段支付类型（1：一阶梯全款支付；2：一阶梯定金支付(全款或定金可选)；3：三阶梯仅定金支付） -->
			  				  <strong >￥<fmt:formatNumber groupingUsed="false" maxFractionDigits="2" minFractionDigits="2" value="${cart.price / 1000 }"/></strong>
			  			  <!--增加预售金额显示 end-->
            <span class="ml20">
				 			    	x ${cart.num}
				 			</span>
        ...
        <!--以下为京东配送方式-->
         ...
         <!--配送方式-->
                  ...
                  <div class="mode-tab-nav">
                    <ul>
                                              <li class="mode-tab-item " id="jd_shipment_item" onclick="choseSendType('pay')">
							<span id="jdShip-span-tip" class="m-txt">京东快递<i class='qmark-icon qmark-tip' data-tips='由京东公司负责配送，速度很快，还接受上门刷卡付款服务'></i></span><b></b>
                        </li>
                                            <li class="mode-tab-item  " id="pick_shipment_item" onclick="choseSendType('picksite')">
						<span class="m-txt">上门自提<i class="qmark-icon qmark-tip" data-tips="自提时付款，支持现金、POS刷卡、支票支付<a href='http://help.jd.com/help/question-64.html' target='_blank' class='ftx-05'>查看自提流程</a>"></i></span><b></b>
                        					...
                        					<div class="mode-tab-con " id="jd_shipment">  
                                                            <ul class="mode-list">
                                                            <li >
                                                            <div class="fore1 hide" id="payPal">
                                           <span class="ftx-03 ">付款方式：</span>&nbsp;&nbsp;<select id="payt" onchange="payt()" style="border: 0;">
                                           	<option>现金</option>
                                           	<option>pos</option>
                                           	<option>支票</option>
                                           </select><br /> 
                                           ...
                                           <!-- <span class="ftx-03">配送时间：</span>&nbsp;&nbsp;工作日、双休日与节假日均可送货</div> -->
                                           	配送时间：&nbsp;&nbsp;<select id="sendt" style="border: 0;" onchange="sendTime()">
                                           		<option value="工作日、双休日与节假日均可送货">工作日、双休日与节假日均可送货</option>
                                           		<option value="2015年4月19日送货">2015年4月19日送货</option>
                                           		<option value="2015年4月20日送货">2015年4月20日送货</option>
                                           		<option value="2015年4月21日送货">2015年4月21日送货</option>
                                           		<option value="2015年4月22日送货">2015年4月22日送货</option>
                                           		<option value="2015年4月23日送货">2015年4月23日送货</option>
                                           	</select>
                                           </div>
                                         </li>				
    								</ul>
...
<div class="form picksite-box">
	...
	<div class="item">
		...
		<div class="fl">
			...
			<div class="pick-more" >
				<span  class="selfpick_more_link open ftx05"  style="cursor:pointer;">展开更多<b></b></span>
			</div>
		</div>
	</div>
	<div class="item">
...
<script type="text/javascript">
	...
	<input type="hidden" id="regionId_default" value=""/> <!-- 默认的区域id -->
	<input type="hidden" id="flag_regionId" value="1"/> <!-- 默认的区域id -->
...
<div class="order-summary">
	<!--  预售 计算支付展现方式 begin -->
		...
		<div class="list">
			...
			<em class="price" id="warePriceId" v="182.60">￥${totalprice }</em>
		</div>
		<div class="list">
			<span>返现：</span>
    ...
    <div class="list" id="showFreeFreight"  style="display:none;" >
      <span>运费优惠：</span><em class="price" id="freeFreightPriceId"> -￥0.00</em>
		...
		<div class="list" >
			<span>应付总额：</span>
			<em class="price" id="sumPayPriceId"> ￥${totalprice } - 20</em>
		</div>
	</div>
		<div class="clr"></div>
	</div>
...
<!-- 隐藏表单，在点击提交订单的时候，保存该数据到数据库 -->
	<form action="" class="hide" id="payAndSendForm">
		<input id="payAndSendId" type="hidden" name="payAndSendId" value=""/>
		<input id="savePara_paymentId" type="hidden" name="paytype" value="4"/>
		<input id="sendtype" type="hidden" name="sendtype" value="pay"/>
		<input id="moneytype" type="hidden" name="moneytype" value="现金"/>
		<input id="pickname" type="hidden" name="pickname" value=""/>
...
<!-- footer end -->
	...
	function chosePayType(type){  //选择支付方式
		if(type == '1'){
			$("#payPal").attr("class","fore1");
			$("#jdshipdate_eidt_id1").attr("class","fore2");
		}else{
			$("#payPal").attr("class","fore1 hide");
			$("#jdshipdate_eidt_id1").attr("class","fore2 hide");
		}
		$("#savePara_paymentId").val(type);
	}
	
	function choseSendType(flag){
		 if ("pay" == flag && $("#jd_shipment_item").hasClass("curr") == false) {
			  ...
			  //window.location.href = "/";//orderUrl + "?rid=" + Math.random(); //请求加载自提信息，配送时间等等
				var city = $("input[name='orderShipping.receiverCity']").val();  //上海
				var regionname = $("input[name='orderShipping.receiverDistrict']").val(); //闵行
				showRegionList(city,regionname);
				var regionId = $("#regionId_default").val();
				var flag = $("#flag_regionId").val();
				showPickname(regionId,flag);
			    //点击自提table标签保存默认自提点
			    doSavePickSite();
			  }
			  $("#sendtype").val(flag);
...
/* 	function changeSendTime(){
	...
	function changeSendTime(venderId) {
	...
	function showRegionList(city,regionname){
		//加载区域选择列表
		
		$.post("/service/region/list",{"city":city},function(data){
			var select = $("#pickRegion_select");
			select.empty();
			var data = eval("("+data+")");
			var array = data.data;
			for(var i=0;i<array.length;i++){
				...
				if(regionname==array[i].name){
					$("#regionId_default").val(array[i].id);
				}
			}
		});
	}
	
	function showPickname(regionId,flag){
		if(flag==1){
			$.post("/service/pickview/list",{"regionId":regionId},function(data){			
				var data = eval("("+data+")");
				var array = data.data;
				var showPickSite = "<span class='ftx-03'>自提地点：</span>" + array[0].name;
				$("#selfpick_name").html(showPickSite);
				$("#pickname").val(array[0].name);
			});
			if($("#pickname").val()!=''){
				$("#flag_regionId").val(0);
			}
